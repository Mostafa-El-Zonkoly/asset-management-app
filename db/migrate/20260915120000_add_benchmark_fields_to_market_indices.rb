...
#   * source_identifier — optional full fetch URL (overrides the built one).
#   * index_kind    — "price" (published price index) or "total_return", so the UI
#                     can label whether the series includes dividends (spec #15).
#
# NOTE: `fetch_code` is added by a SEPARATE later migration
# (AddFetchCodeToMarketIndices) so environments that already ran THIS migration
# still pick it up — do not add it here.
#
