...
export default class extends Controller {
  ...
  async load(url) {
    ...
    const leftBounds = boundsFor(datasets[0]?.data)
    const rightBounds = boundsFor(datasets[1]?.data)
    ...
    const scales = {
      ...
      y: {
        ...
        title: {
          display: true,
          text: datasets[0] ? `${datasets[0].label} ${data.y_axis_label}` : data.y_axis_label,
        },
        ...(leftBounds ? { min: leftBounds.min, max: leftBounds.max } : {}),
      },
    }
    ...
    if (datasets.length === 2) {
      scales.y1 = {
        ...
        title: {
          display: true,
          text: `${datasets[1].label} ${data.y_axis_label}`,
        },
        ...(rightBounds ? { min: rightBounds.min, max: rightBounds.max } : {}),
